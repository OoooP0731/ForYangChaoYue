#!/usr/bin/env python3
"""Segment-level training with WavLM-Large on the MODMA corpus.

- Loads all .wav files listed in the metadata Excel (HC/MDD labels).
- Chunks each recording into 7s windows with 20% overlap.
- Shuffles all segments, splits into train/val/test by ratio, and trains
  a classifier on top of frozen-then-unfrozen WavLM-Large.
- Saves best checkpoints for segment-F1 and subject-F1 (aggregated per subject).
- Early stopping triggers only after 5 epochs of no segment-F1 improvement *and*
  once best validation segment accuracy reaches at least 0.8.
- Test metrics (segment+subject) are saved to JSON.
"""
...
def build_segment_samples(file_paths: List[str], labels: List[int], subjects: List[str], data_cfg: DataConfig) -> List[Dict]:
    seg_len = data_cfg.sample_rate * data_cfg.segment_duration
    hop = max(1, int(seg_len * (1 - data_cfg.overlap_ratio)))
    samples: List[Dict] = []
    subj_counts = Counter()
    iterator = zip(file_paths, labels, subjects)
    for path, lbl, subj in tqdm(list(iterator), desc="Indexing audio", total=len(file_paths)):
        try:
...
def run_training() -> None:
    ...
    paths, labels, subjects = load_metadata(CONFIG.data)
    samples = build_segment_samples(paths, labels, subjects, CONFIG.data)
    rng = random.Random(CONFIG.train.random_seed)
    rng.shuffle(samples)

    # counts
    label_counts = Counter(labels)
    seg_counts = Counter(s["label"] for s in samples)
    logger.info("Subjects | HC=%d | MDD=%d", label_counts[0], label_counts[1])
    logger.info("Segments | HC=%d | MDD=%d | total=%d", seg_counts[0], seg_counts[1], len(samples))

    total = len(samples)
    n_train = int(total * CONFIG.train.train_ratio)
    n_val = int(total * CONFIG.train.val_ratio)
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        if improved:
            ...
            if patience >= CONFIG.train.patience and best_segment_acc >= 0.8:
                logger.info("Early stopping triggered (best segment acc=%.4f)", best_segment_acc)
                break
    ...
    metrics_path = os.path.join(CONFIG.train.log_dir, f"test_metrics_segment_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train WavLM-Large with segment split", add_help=True)
